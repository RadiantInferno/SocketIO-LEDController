...
app.get("/senddata/:mode-:color-:brightness-:speed-:length",function(request, response){
    ...
    if(infoValidFlag) {
        ...
        currentData = data
        console.log("Recieved valid data from GET and updated: " + currentData)
    }
    else{
        response.send(responseString)
        console.log("Recieved invalid data from GET")
    }
});

app.get("/setcolour/:color",function(request, response){
    ...
    if(hex) {
        ...
        io.emit('set-info', data)
        currentData = data
        console.log("Recieved valid color from GET and updated: ")
        console.log(currentData)
    }
    else{
        response.send("unrecognised color")
        console.log("Recieved invalid color from GET: " + request.params.color.replace(/\s/g, ''))
    }
});

app.get("/setmode/:mode",function(request, response){
    var newMode = modeNameToIndex(request.params.mode)
    if(newMode != -1) {
        ...
        response.send("unrecognised mode")
        console.log("Recieved invalid mode from GET: " + request.params.mode.replace(/\s/g, ''))
    }
});
...
function colourNameToHex(colour)
{
    ...
    "yellow":"#ffff00","yellowgreen":"#9acd32","read":"#ff0000"};
...
function modeNameToIndex(mode){
    var modes = {
        "0" : 0, "zero" : 0, "solidcolour" : 0, "solidcolor" : 0, "colour" : 0, "color" : 0,
        "1" : 1, "one" : 1, "rainbow" : 1,
        "2" : 2, "two" : 2, "strobe" : 2,
    ...
    if (typeof modes[mode.toLowerCase().replace(/\s/g, '')] != 'undefined')
        return modes[mode.toLowerCase().replace(/\s/g, '')];
...
function setDataToModeDefaults(mode){
    ...
    switch(mode){
        ...
        case 2:
            ...
            break
        case 3:
            currentData.speed = 50
            currentData.length = 1000
            break
        case 4:
